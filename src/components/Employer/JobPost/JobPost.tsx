...
import { fetchJobById } from '../../../services/Employer/fetJobs'
...
function JobPost() {
    ...
    useEffect(() => {
        const fetch = async () => {
            ...
            setJobData(job.job)
            setSalaryType(job.job.salaryType)
            setFixedSalary(job.job.fixedSalary)
            setRangeSalary({from:job.job.salaryFrom,to:job.job.salaryTo})
            }
            ...
            if (title && category && qualification && deadline && desc && experience && jobType) {
                if (fixedSalary || (from && to)) {
                    ...
                    if (id) {
                        ...
                        const {data} = await api.put(`/employer/editjob/${id}`,{ ...job })
                        navigate('/employer')
                    } else {
    ...
    return (
        <>
            <div className='w-full h-20 md:h-40 top-text-bg-img flex items-center justify-center mt-2'>
                {id ? <h1 className='text-2xl md:text-3xl font-bold font-work'>Edit Job</h1> :
            ...
            <div className='px-2 md:px-40 mt-6 mb-20'>
                <form onSubmit={required} className='w-full pb-8 border border-primary-200 rounded-lg grid grid-cols-1 md:grid-cols-2 px-8 md:gap-2 pt-6'>
                    ...
                    <div>
                        ...
                        <input type="text" className='addjob-text-box' value={jobData.title} name="title" id="" onChange={FormInput} required />
                    ...
                    <div>
                        ...
                        <select name="category" id="" value={jobData.category} className='addjob-text-box' onChange={FormInput}>
                    ...
                    <div>
                        ...
                        <select name="salaryType" id="" className='addjob-text-box' value={salaryType} onChange={FormInput}>
                            ...
                            salaryType === 'Range' ?
                                <div className='flex gap-2'>
                                    <div className='w-full'>
                                        <input type="number" value={rangeSalary.from} className='addjob-text-box ' name="from" id="" placeholder='From' onChange={FormInput} />
                                        <p className='text-red-500 text-xs'>{err.from}</p>

                                    </div>
                                    <div className='w-full'>
                                        <input type="number" value={rangeSalary.to} className='addjob-text-box' name="to" id="" placeholder='To' onChange={FormInput} />
                                        <p className='text-red-500 text-xs'>{err.to}</p>

                                    </div>

                                </div>
                                : <div> <input type="number" value={fixedSalary} className='addjob-text-box' name="fixed" id="" onChange={FormInput} />
                    ...
                    <div>
                        ...
                        <input type="text" className='addjob-text-box' value={jobData.qualification} name="qualification" id="" onChange={FormInput} required />
                        <p className='text-red-500 text-xs'>{err.qualification}</p>

                    </div>
                    <div>
                        <h1 className='text-sm'>Experience Level</h1>
                        <select name="experience" id="" className='addjob-text-box' value={jobData.experience} onChange={FormInput}>
                    ...
                    <div>
                        ...
                        <input type="date" min={tomorrowDate} value={jobData.deadline} className='addjob-text-box' name="deadline" id="" onChange={FormInput} required />
                    </div>
                    <div>
                        <h1 className='text-sm'>Job Type</h1>
                        <select name="jobType" id="" value={jobData.jobType} className='addjob-text-box' onChange={FormInput}>
                    ...
                    <div className='md:col-span-2'>
                        ...
                        <textarea name="desc" id="" className=' addjob-text-box h-40' value={jobData.desc} onChange={FormInput} required></textarea>
                        <p className='text-red-500 text-xs'>{err.desc}</p>

                    </div>
                    <div>
                        <button type='submit' className='ms-2 px-2 rounded-lg py-2 bg-primary-900 text-white'>{id ?'Update job' :'Post Now'}</button>
                    </div>
                </form>
